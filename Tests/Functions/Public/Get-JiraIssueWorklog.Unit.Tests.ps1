    ...
    Describe "Behavior" {
        Context "Behavior testing" {
            ...
            It "Obtains all Jira worklogs from a Jira issue if the Jira object is provided" {
                $issue = Get-TestJiraIssue -JiraServer $jiraServer -ID $issueID -Key $issueKey
            ...
            It "Handles pipeline input from Get-JiraIssue" {
                $worklogs = Get-TestJiraIssue -JiraServer $jiraServer -ID $issueID -Key $issueKey | Get-JiraIssueWorklog
