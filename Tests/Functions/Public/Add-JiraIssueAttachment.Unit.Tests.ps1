...
"@
        ...
        Mock Get-JiraIssue -ModuleName JiraPS {
            ...
            Get-TestJiraIssue -Key $issueKey -RestUrl "$jiraServer/rest/api/2/issue/$issueKey"
        }
    ...
    Describe "Behavior" {
        It "calls all necessary functions under the hood" {
            $null = Add-JiraIssueAttachment -Issue (Get-TestJiraIssue -Key $issueKey -RestUrl "$jiraServer/rest/api/2/issue/$issueKey") -FilePath $filePath
    ...
    Describe "Input Validation" {
        Context "Type Validation - Positive Cases" {
            ...
            It "issue can be an Issue object" {
                { Add-JiraIssueAttachment -Issue (Get-TestJiraIssue -Key $issueKey -RestUrl "$jiraServer/rest/api/2/issue/$issueKey") -FilePath $filePath } | Should -Not -Throw
            }
