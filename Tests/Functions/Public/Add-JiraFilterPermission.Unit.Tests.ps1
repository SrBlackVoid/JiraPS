...
"@
        ...
        Mock Get-JiraFilter -ModuleName JiraPS {
            ...
            foreach ($_id in $Id) {
                Get-TestJiraFilter -ID $_id -JiraServer $jiraServer
            }
        }
    ...
    Describe "Behavior" {
        BeforeAll {
            Mock ConvertTo-JiraFilter -ModuleName JiraPS {
                ...
                [PSCustomObject]@{ PSTypeName = 'JiraPS.Filter' }
            }
        }

        Context "Filter Object Input" {
            It "Adds share permission to Filter Object" {
                $filter = Get-TestJiraFilter -ID 12844 -JiraServer $jiraServer
                {
    ...
    Describe "Input Validation" {
        ...
        Context "Type Validation - Positive Cases" {
            It "accepts JiraPS.Filter objects" {
                { Add-JiraFilterPermission -Filter (Get-TestJiraFilter -ID 1 -JiraServer $jiraServer) -Type "Global" } | Should -Not -Throw
            }
        ...
        Context "Pipeline Support" {
            It "allows a JiraPS.Filter to be passed over the pipeline" {
                { Get-TestJiraFilter -ID 1 -JiraServer $jiraServer | Add-JiraFilterPermission -Type "Global" } | Should -Not -Throw
            }
        ...
        Context "Multiple Items" {
            It "can process multiple Filters" {
                $filters = 1..5 | ForEach-Object { Get-TestJiraFilter -ID 1 -JiraServer $jiraServer }
